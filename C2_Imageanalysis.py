...
# 41. Morphological operations (erode, dilate)
...
# 47. Bounding box detection
bbox_img = img.copy()
for cnt in contours:
    x,y,w,h = cv2.boundingRect(cnt)
    cv2.rectangle(bbox_img, (x,y), (x+w,y+h), (255,0,0), 2)

# 48. Circle detection (Hough Circle)
...
# 50. Background subtraction
bg = cv2.createBackgroundSubtractorMOG2()
fgmask = bg.apply(img)

# 51. Image segmentation (simple threshold)
seg = cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,
                             cv2.THRESH_BINARY,11,2)

# 52. Adaptive thresholding
